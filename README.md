...
Μια εφαρμογή αυτοματισμού για τη δυναμική δημιουργία και διαχείριση Email Newsletters. Το εργαλείο συνδέεται με το Google Sheets API για τη λήψη δεδομένων και τα μετατρέπει αυτόματα σε έτοιμο κώδικα HTML, χρησιμοποιώντας επαναχρησιμοποιήσιμα components.
...
- Fallback Logic: Αυτόματη συμπλήρωση στοιχείων για διασφάλιση του Accessibility.
...
3. Δημιουργήστε ένα αρχείο .env στο root και προσθέστε το SPREADSHEET_ID σας.  
4. Τοποθετήστε το αρχείο credentials.json του Google Cloud στον αρχικό φάκελο.
5. Για Live Preview (Browser): Τρέξτε τον τοπικό server για να βλέπετε τις αλλαγές σας στο http://localhost:3000:
- Με τοπικά δεδομένα (JSON): ```npm run dev```
- Με δεδομένα από Google Sheets: ```node scripts/build.js sheet```
6. Για δημιουργία τελικού αρχείου (Build): Για να παραχθεί το στατικό HTML αρχείο στον φάκελο /dist:
- Χρήση τοπικών δεδομένων: ```npm run build```
- Χρήση δεδομένων Google Sheets: ```npm run build:sheet```
7. Χρήση: Μόλις ολοκληρωθεί το build, ανοίξτε το παραγόμενο αρχείο .html από τον φάκελο dist, αντιγράψτε τον κώδικα και επικολλήστε τον στον HTML editor της πλατφόρμας αποστολής σας (π.χ. Contact Pigeon).
